...
from typing import Dict, Optional, List, Any # Añadido List, Any
...
try:
    ...
    print(f"CRITICAL ERROR en SAM/common/utils/mail_client.py: No se pudieron importar módulos de 'common.utils'.", file=sys.stderr)
...
class EmailAlertClient:
    def __init__(self, service_name: str = "SAM"):
        ...
        if not all([self.smtp_server, self.from_email, self.recipients]):
            logger.error("Configuración de email incompleta (falta smtp_server, from_email o recipients). Las alertas por email podrían no funcionar.")
