...
class AutomationAnywhereClient:  # Nombre original de tu clase en SAM
    ...
    def __init__(
        ...
        self.url_base: str = control_room_url.strip("/")
        self.username: str = username
        self.password: str = password
        self.api_key: str | None = api_key
        self.callback_url_for_deploy: str | None = callback_url_for_deploy

        self._timeout_requests: int = api_timeout_seconds
        self._token_refresh_buffer_seconds: int = token_refresh_buffer_sec
        self._default_page_size: int = default_page_size
        self._max_pagination_pages: int = max_pagination_pages  # Límite de páginas a solicitar
    ...
    def _manejar_excepcion_api(
        self, error: requests.exceptions.RequestException, response_obj: Optional[requests.Response] = None, request_data=None
    ):
        """Maneja excepciones de requests y errores HTTP, levantando una excepción formateada."""
        error_message = ""
        status_code_str: str = str(response_obj.status_code) if response_obj is not None else "N/A"
        api_details = ""
        ...
        if response_obj is not None:
            try:
                ...
                api_details: str = f"API Response (Status {status_code_str}): Code='{api_code}', Message='{api_message}'"
            except ValueError:
        ...
        req_data_summary: str = str(request_data)[:250] if request_data else "N/A"  # Más contexto del request

        if isinstance(error, requests.exceptions.Timeout):
            error_message: str = f"API request timed out after {self._timeout_requests}s. {api_details}"
    ...
    def _obtener_token(self) -> Optional[str]:
        ...
        url: str = f"{self.url_base}{self._ENDPOINT_AUTH_V2}"
        payload = {
    ...
    def _es_valido_el_token(self) -> bool:
        ...
        if not self._token_auth or not self._ts_inicio_token:
            return False
        elapsed_time: float = (datetime.now() - self._ts_inicio_token).total_seconds()
    ...
    def _realizar_peticion_api(
        ...
        url: str = f"{self.url_base}{endpoint}"
        final_headers = {}
        ...
        request_kwargs = {"params": params_query, "json": json_payload, "data": data_payload, "timeout": self._timeout_requests}
        if final_headers:
            request_kwargs["headers"] = final_headers

        response_obj = None
        try:
            payload_summary: str = str(json_payload or data_payload)[:250]  # Un poco más de info
    ...
    def _procesar_pagina_entidades(self, respuesta_api: Dict[str, Any], campos_api_a_extraer: List[str]) -> List[Dict[str, Any]]:
        """Procesa una página de entidades de la API y extrae los campos especificados."""
        entidades_pagina_api = respuesta_api.get(self._FIELD_LIST, [])
        return [{campo: entidad_api.get(campo) for campo in campos_api_a_extraer} for entidad_api in entidades_pagina_api]

    def _obtener_total_entidades(self, respuesta_api: Dict[str, Any]) -> int:
        """Obtiene el total de entidades esperadas de la respuesta de la API."""
        page_info = respuesta_api.get(self._FIELD_PAGE, {})
        total = page_info.get("totalFilter")
        if total is None:
            total = page_info.get("total")
        return total if isinstance(total, int) else -1

    def _obtener_lista_paginada_entidades(
        self, endpoint: str, payload_base: dict, campos_api_a_extraer: List[str], limite_por_pagina: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        while paginas_intentadas < self._max_pagination_pages:
            ...
            try:
                logger.debug(f"Paginación: Solicitando página {paginas_intentadas}, offset: {offset_actual}, page_size: {page_size}")
                respuesta_api = self._realizar_peticion_api("POST", endpoint, json_payload=payload_base_actual)
                entidades_pagina = self._procesar_pagina_entidades(respuesta_api, campos_api_a_extraer)

                if not primera_pagina_procesada:
                    total_entidades_esperadas = self._obtener_total_entidades(respuesta_api)
                    primera_pagina_procesada = True
                    logger.info(f"Paginación: Total esperado: {total_entidades_esperadas if total_entidades_esperadas != -1 else 'No disponible'}")

                if not entidades_pagina:
                    logger.info(f"Paginación: No más entidades en offset {offset_actual}. Finalizando.")
                    break

                lista_completa.extend(entidades_pagina)
                logger.debug(f"Paginación: Obtenidos {len(entidades_pagina)} entidades. Total: {len(lista_completa)}")

                if total_entidades_esperadas != -1 and len(lista_completa) >= total_entidades_esperadas:
                    logger.info(f"Paginación: Completado ({len(lista_completa)}/{total_entidades_esperadas}).")
                    break

                if len(entidades_pagina) < page_size:
                    logger.info(f"Paginación: Última página ({len(entidades_pagina)} < {page_size}).")
                    break

                offset_actual += page_size

            except Exception as e:
                logger.error(f"Paginación: Error en página {paginas_intentadas}: {e}", exc_info=True)
                break

        if paginas_intentadas >= self._max_pagination_pages:
            logger.warning(f"Paginación: Límite de {self._max_pagination_pages} páginas alcanzado. Obtenidos: {len(lista_completa)}")

        logger.info(f"Paginación: Total final: {len(lista_completa)} entidades.")
    ...
    def desplegar_bot(self, file_id: int, run_as_user_ids: List[int], bot_input: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                "device is busy" in error_lower
                or (
                    "400 client error" in error_lower
                    and "invalid_argument" not in error_lower
                    and "user" not in error_lower
                    and "deleted" not in error_lower
                    and "disabled" not in error_lower
                )
                or "412 client error" in error_lower
            ...
            for item_api in actividad_lista_api:
                ...
                if detalle.get("userId") is not None:
                    try:
                        ...
                        logger.warning(
                            f"No se pudo convertir userId '{detalle['userId']}' a entero para deploymentId '{detalle.get('deploymentId')}'."
                        )
    ...
    def obtener_usuarios_detallados(
        self, filtro_descripcion_contiene: Optional[str] = None, user_ids: Optional[List[int]] = None
    ) -> List[Dict[str, Any]]:
        """Obtiene detalles de usuarios de A360, con paginación y opción de filtrar por IDs."""
        logger.info(
            f"Obteniendo usuarios detallados (desc: '{filtro_descripcion_contiene}', user_ids: {str(user_ids)[:50] if user_ids else 'Todos'})..."
        )
        endpoint = self._ENDPOINT_USERS_LIST_V2

        payload_base = {self._FIELD_PAGE: {}, self._FIELD_SORT: [{"field": "username", "direction": "asc"}]}

        operands = []
        if filtro_descripcion_contiene:
            operands.append({self._FIELD_OPERATOR: "substring", self._FIELD_FIELD: "description", self._FIELD_VALUE: filtro_descripcion_contiene})
        if user_ids:
            operands.append(
                {
                    self._FIELD_OPERATOR: "or",
                    self._FIELD_OPERANDS: [
                        {self._FIELD_OPERATOR: "eq", self._FIELD_FIELD: self._FIELD_ID, self._FIELD_VALUE: uid} for uid in user_ids
                    ],
                }
            )
        ...
        if operands:
            ...
            if len(operands) == 1:
                payload_base[self._FIELD_FILTER] = operands[0]
        ...
        for usr_api in usuarios_api_bruto:
            ...
            if usr_api.get("licenseFeatures"):
                ...
                if isinstance(features, list) and features:
                    licencia_str = features[0]
                elif isinstance(features, str):
                    licencia_str = features
    ...
    def obtener_robots(
        ...
        operands_list = [{self._FIELD_OPERATOR: "eq", self._FIELD_FIELD: "type", self._FIELD_VALUE: "application/vnd.aa.taskbot"}]

        if filtro_path_base:
            operands_list.append({self._FIELD_OPERATOR: "substring", self._FIELD_FIELD: "path", self._FIELD_VALUE: filtro_path_base})

        if filtro_nombre_prefijo:
            operands_list.append(
                {
                    self._FIELD_OPERATOR: "substring",
                    self._FIELD_FIELD: "name",
                    self._FIELD_VALUE: filtro_nombre_prefijo,
        ...
        payload_base = {
            ...
            self._FIELD_FILTER: {self._FIELD_OPERATOR: "and", self._FIELD_OPERANDS: operands_list},
        }
        ...
        robots_api_bruto = self._obtener_lista_paginada_entidades(endpoint, payload_base, campos_api_a_extraer)
        ...
        for bot_api in robots_api_bruto:
            ...
            elif nombre_bot and filtro_nombre_prefijo and nombre_bot.startswith(filtro_nombre_prefijo) and not patron_nombre_robot.match(nombre_bot):
