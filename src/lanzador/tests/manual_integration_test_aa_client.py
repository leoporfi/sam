...
def run_manual_test():
    ...
    callback_url = get_input("URL de Callback para despliegues (opcional)", "http://localhost:8080/callback")
    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                status_filter = get_input(
                    "Filtrar por status (ej. CONNECTED, DISCONNECTED, dejar vacío para todos los que permite el método)", "CONNECTED"
                )
                ...
                usuarios = client.obtener_usuarios_detallados_para_sam(filtro_descripcion_contiene=desc_filter, user_ids=user_ids_list)
