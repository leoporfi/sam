...
@component
def HeaderNav(theme_is_dark: bool, on_theme_toggle):
    """Header de navegación que usa link() - debe estar dentro del contexto del router."""
    return html.header(
...
@component
def PageWithLayout(theme_is_dark: bool, on_theme_toggle, children):
    """Wrapper que incluye el header y la estructura principal en cada página."""
    return html._(
        HeaderNav(theme_is_dark=theme_is_dark, on_theme_toggle=on_theme_toggle),
...
@component
def DashboardPage_old(theme_is_dark: bool, on_theme_toggle):
...
def DashboardPage(theme_is_dark: bool, on_theme_toggle):
    """Lógica y UI para la página principal de Robots."""
    robots_state = use_robots()
    # Importar el hook de equipos para la sincronización
