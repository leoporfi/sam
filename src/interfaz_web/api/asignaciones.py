...
router = APIRouter(
    prefix="/api/robots/{robot_id}/asignaciones",
    tags=["Asignaciones"]
)

@router.get("/")
def get_robot_asignaciones(
    robot_id: int,
...
@router.post("/")
def update_robot_asignaciones(
    robot_id: int, 
    update_data: AssignmentUpdateRequest,
    db: DatabaseConnector = Depends(get_db_connector)
):
    try:
        result = asignacion_service.update_asignaciones_robot(
            db, 
            robot_id, 
            update_data.assign_team_ids, 
            update_data.unassign_team_ids
        )
        return result
