...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Mantenimiento_MoverAHistorico]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[Mantenimiento_MoverAHistorico] AS'
END

-- Inicio de dbo_Mantenimiento_MoverAHistorico.sql
ALTER   PROCEDURE [dbo].[Mantenimiento_MoverAHistorico]
