    ...
    def _limpiar_asignaciones_robots_no_candidatos(self,
        ...
        for robot_id in robot_ids_con_asignaciones_dinamicas:
            ...
            if not es_candidato_actual_para_balanceo:
                ...
                can_sd, just_sd = self.cooling_manager.can_scale_down(robot_id, tickets_log_no_candidato, num_actual_din_no_candidato)
                ...
                if can_sd:
                    ...
                    if liberados_count > 0:
                        self.cooling_manager.register_scale_down(robot_id, tickets_log_no_candidato, liberados_count)
                        self.historico_client.registrar_decision_balanceo(
                            robot_id, tickets_log_no_candidato, 
                            num_actual_din_no_candidato, num_actual_din_no_candidato - liberados_count,
                            "DESASIGNAR_INACTIVO_OFFLINE", 
                            f"Robot no Act/On. Liberados:{liberados_count}. JustCool:{just_sd}"
                        )
                    if liberados_count < len(equipos_asignados_al_robot_no_candidato):
                        logger.warning(f"Fase 0: No se pudieron liberar todos los equipos de RobotId {robot_id} (no candidato).")
                else:
        ...
        logger.info("Balanceo - Fase 0: Limpieza de Asignaciones completada.")
            ...
            try:
                ...
                for r_id, r_cfg in mapa_config_robots.items(): # Solo itera sobre robots Activos y Online
                    ...
                    if num_actual_din < min_req:
                        ...
                        can_add, just_add_min = self.cooling_manager.can_scale_up(r_id, tickets, num_actual_din)
                        if can_add:
                ...
                for r_id_req, faltante_req, prio_req, cfg_req, tks_req, num_asig_antes_req_fase1 in min_needs_list:
                    ...
                    while equipos_asignados_a_req_en_fase1 < faltante_req:
                        ...
                        if equipos_libres_en_pool_ahora:
                            ...
                            can_add_req_sub, just_add_req_sub = self.cooling_manager.can_scale_up(r_id_req, tks_req, num_actual_din_req_antes_asign)
                            if can_add_req_sub:
                                if self._realizar_asignacion_db(r_id_req, equipo_a_asignar, "ASIGNAR_MIN_POOL"):
                                    ...
                                    self.cooling_manager.register_scale_up(r_id_req, tks_req, 1)
                                    self.historico_client.registrar_decision_balanceo(
                                        r_id_req, tks_req, 
                        ...
                        if not equipo_conseguido_para_req_subintento and (equipos_asignados_a_req_en_fase1 < faltante_req) : # Aún necesita y no se obtuvo del pool
                            ...
                            for r_id_donor_loop, cfg_donor_loop in mapa_config_robots.items(): # Donantes deben ser Activos y Online
                                ...
                                if num_actual_din_donor > min_req_donor:
                                    ...
                                    can_remove_donor, just_rem_donor = self.cooling_manager.can_scale_down(r_id_donor_loop, tks_donor_loop, num_actual_din_donor)
                                    if can_remove_donor:
                            ...
                            if potential_donors:
                                ...
                                if eq_a_robar:
                                    ...
                                    if self._realizar_desasignacion_db(r_id_d_sel, eq_a_robar):
                                        ...
                                        self.cooling_manager.register_scale_down(r_id_d_sel, donor_sel["tickets"], 1)
                                        self.historico_client.registrar_decision_balanceo(
                                        ...
                                        can_add_req_robado, just_add_req_robado = self.cooling_manager.can_scale_up(r_id_req, tks_req, num_actual_din_req_antes_robado)
                ...
                for r_id, r_cfg in mapa_config_robots.items(): # Solo itera sobre robots Activos y Online
                    ...
                    if num_actual_din > target_desasignacion:
                        can_sd, just_sd = self.cooling_manager.can_scale_down(r_id, tickets, num_actual_din)
                        if can_sd:
                            ...
                            if liberados_f2_count > 0:
                                self.cooling_manager.register_scale_down(r_id, tickets, liberados_f2_count)
                ...
                for r_id, r_cfg in mapa_config_robots.items(): # Solo robots Activos y Online
                    ...
                    if num_actual_din_post_f2 < limite_sup:
                        ...
                        can_add_f3, just_add_f3 = self.cooling_manager.can_scale_up(r_id, tickets, num_actual_din_post_f2)
                        if can_add_f3:
                ...
                for r_id, nec_adic, prio, cfg, tks, num_asig_antes_f3 in necesidades_adicionales_final:
                    ...
                    if asignados_f3_count > 0:
                        self.cooling_manager.register_scale_up(r_id, tks, asignados_f3_count)
                        self.historico_client.registrar_decision_balanceo(
